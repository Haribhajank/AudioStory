...
CACHE_PATH = SCRIPT_DIR / "output" / "prompt_cache.json"
...
def story_thumnail_gemini(gemini_prompt):
  try:
...
# }
    ...
    raw_response = story_prompts(system_prompt, user_prompt)
    if raw_response is None:
        print("[!] OpenAI response error!")
        print("Using gemini instead...")
        raw_response = story_thumnail_gemini(prompts.gemini_prompt % (master_doc_content, config.N_PROMPTS))
        if raw_response is None:
            print("[!] Gemini response error!")
            return
    ...
    if raw_response.startswith("```"):
        prompt_data = raw_response.strip("`").strip("json").strip()
