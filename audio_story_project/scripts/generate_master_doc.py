...
from prompts.create_master_doc import system_story_prompt, user_story_prompt, gemini_story_prompt
...
def normalize_master_doc(doc):
    return {
        "characters": doc.get("Story Characters") or doc.get("story characters"),
        "plot": doc.get("Story Plot") or doc.get("story plot"),
        "personas": {"persona": "dummy_perasona"}, #no need
        "recap": {},
        "title": doc.get("Story Title"),
        "author": "KUKU FM"
    }
...
def create_master_doc(idea, num_episodes,time_per_episode,genre):
    ...
    if not response:
        print(" gpt request failed. Using Gemini.")
        messages = [gemini_story_prompt % (idea, num_episodes, time_per_episode, genre)]
        print(" Using Gemini key:", os.getenv("GEMINI_API_KEY"))
        response = gemini_request(messages)
    if not response:
        print("Both LLM request failed.")
        return
