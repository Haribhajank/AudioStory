...
export default function Home() {
  ...
  const [timePerEpisode, setTimePerEpisode] = useState("");
  ...
  const handleSubmit = () => {
    ...
    localStorage.setItem("idea", idea);
    localStorage.setItem("numEpisodes", numEpisodes);
    localStorage.setItem("timePerEpisode", timePerEpisode);
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full px-4 text-white text-center">
        <div className="flex flex-col gap-6 items-center -mt-36 sm:-mt-44 md:-mt-52 w-full px-4 max-w-2xl">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full">
            <input
              ...
              className="w-full px-4 py-3 text-black placeholder-gray-700 bg-white/60 rounded-xl shadow-inner backdrop-blur-lg border border-white/40 focus:outline-none focus:ring-2 focus:ring-white text-base"
            />
            <input
              type="text"
              value={timePerEpisode}
              onChange={(e) => setTimePerEpisode(e.target.value)}
              placeholder="Time per episode (e.g. 10 min)"
              className="w-full px-4 py-3 text-black placeholder-gray-700 bg-white/60 rounded-xl shadow-inner backdrop-blur-lg border border-white/40 focus:outline-none focus:ring-2 focus:ring-white text-base"
            />
          </div>
          ...
          <input
            ...
            placeholder="Genre (e.g. sci-fi, fantasy, romance)"
            className="w-full px-4 py-3 text-black placeholder-gray-700 bg-white/60 rounded-xl shadow-inner backdrop-blur-lg border border-white/40 focus:outline-none focus:ring-2 focus:ring-white text-base"
          />
          ...
          <button
            ...
            className="w-full sm:w-auto px-6 py-4 bg-gradient-to-br from-black to-gray-800 text-white font-semibold rounded-xl hover:from-gray-900 hover:to-black transition duration-200 shadow-lg"
          >
            Submit
          </button>
        </div>
